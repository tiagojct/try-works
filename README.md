...
Each regenerates from `try-works.json`:

```
css/                 custom properties (colours, type scale, spacing), both modes
tailwind/            preset: colours, fontSize, spacing, borderRadius
typst/               slide theme + generated colours
web/                 minimal 11ty starter
obsidian/            theme.css + manifest (Try-Fire / True Lamp)
themes/terminals/    Ghostty preset (lit)
vscode/              dark code theme, tuned for R and Python
```

Run `make generate` and all of them rebuild. `make cvd` runs the colour-vision
check.
...
R: source r/tryworks.R for scale_colour_tryworks_d, _c, _div, their fill
equivalents, and theme_tryworks(mode = "light" or "dark").

Python: import python/tryworks.py and call use_tryworks(); it registers the
...
print/SPEC.md is generated from the json: a recommended profile (PSO Coated v3 /
...
typst/poster.typ is a poster preset with a page sized to trim plus 3 mm bleed,
crop marks, and a 5 mm safe-area guide, defaulting to the fire-as-one-mark
layout. A poster proof showing the geometry is in specimen/poster-proof.svg.

## Accessibility

Body contrast is locked at build time; this layer covers the rest, in
css/a11y.css. Interactive elements get a visible :focus-visible ring in the
...
css/fallbacks.css declares a local() fallback per face with size-adjust and
...
css/motion.css carries four durations (fast 120ms to slower 480ms) and four
...
specimen/motion.html for the curves.

## Brand

The identity is type-forward: the Try-Works wordmark in Fraunces is the
primary mark, with an ember emblem (assets/logo.svg) as a square secondary for
...
separately (fonts/README.md). If you redistribute subsetted fonts, ship the OFL
...
changelog, tag. CONTRIBUTING.md and PUBLISHING.md cover the rest.
...
quarto/README.md and quarto/example.
...
classes in `css/typography.css`: display, headline, title, subhead, body-lg,
...
production. `specimen/type-specimen.html` shows the ladder, the optical-size
...
Mono, all OFL; see `fonts/README.md`.


## Accessibility

Every body pair clears WCAG-AA in both modes, and a Machado-2009 colour-vision
pass (`scripts/cvd_check.py`) covers the code hues. The identity hues separate
...
`try-works.json` is the only file you edit. Everything else is generated.

```
make generate   # rebuild all surfaces from the json
